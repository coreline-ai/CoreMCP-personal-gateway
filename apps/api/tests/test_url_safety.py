...
def test_host_allowlist_still_allows_host_but_not_scheme_host_port_or_path_changes(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_downstream_does_not_pin_host_allowlist_entries(tmp_path: Path):
    ...
    assert observed["url"] == "http://fake.local/mcp"
