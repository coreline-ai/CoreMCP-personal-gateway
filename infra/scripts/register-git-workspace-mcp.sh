...
GIT_WORKSPACE_ROOT="${GIT_WORKSPACE_ROOT:-/Users/hwanchoi/projects}"
GIT_WORKSPACE_APP_DIR="${GIT_WORKSPACE_APP_DIR:-$PROJECT_ROOT/apps/git-workspace-mcp}"
PYTHON3="${GIT_WORKSPACE_PYTHON:-$(command -v python3)}"

if [[ ! -s "$ADMIN_TOKEN_FILE" ]]; then
  echo "Admin token not found: $ADMIN_TOKEN_FILE" >&2
  exit 65
fi
if [[ ! -d "$GIT_WORKSPACE_ROOT" ]]; then
  echo "GIT_WORKSPACE_ROOT is not a directory: $GIT_WORKSPACE_ROOT" >&2
  exit 66
fi
if [[ ! -d "$GIT_WORKSPACE_APP_DIR/git_workspace_mcp" ]]; then
  echo "Git Workspace MCP package not found: $GIT_WORKSPACE_APP_DIR" >&2
  exit 66
fi
...
export GIT_WORKSPACE_ROOT GIT_WORKSPACE_APP_DIR PYTHON3
...
workspace_root = os.environ["GIT_WORKSPACE_ROOT"]
app_dir = os.environ["GIT_WORKSPACE_APP_DIR"]
...
payload = {
    ...
    "slug": slug,
    "description": "Read-only git inspection across /Users/hwanchoi/projects (status, log, diff, blame, branches, activity).",
    "transport_type": "stdio",
    "endpoint_url": "stdio://git_workspace",
    "auth_type": "none",
    "category": "personal-docs",
    "stdio_command": python3,
    "stdio_args": ["-m", "git_workspace_mcp.main"],
    "stdio_env": {"GIT_WORKSPACE_ROOT": workspace_root},
    "stdio_cwd": app_dir,
...
git_tools = [item.get("name") or item.get("exposed_name") for item in tools.get("items", []) if "git_workspace" in str(item.get("name") or item.get("exposed_name"))]
print("git workspace tools:", ", ".join(git_tools[:10]) or "<none visible yet>")
PY
