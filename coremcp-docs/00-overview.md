...
CoreMCP는 본인이 Mac mini에서 운영하는 protected MCP gateway다. 외부 AI 클라이언트(Codex CLI `exec` 우선, 필요 시 Claude Code/ChatGPT/Cursor)는 `/mcp` endpoint 하나만 등록하면 사용자의 도구함에 담긴 모든 downstream MCP tool을 사용할 수 있다.
...
### 3.2 우선 클라이언트
- Codex CLI `exec` (Mac mini 로컬)
- Claude Code (옵션: Mac mini 로컬 + Tailscale 통한 MacBook)
- 추후: Claude desktop, ChatGPT custom MCP, Cursor, OpenClaw (14-mcp-client-profiles.md)

모든 client는 정적 bearer/client token으로 시작. Codex CLI exec는 `COREMCP_CLIENT_TOKEN` 환경변수를 통해 client token을 주입하고, 여러 client를 운영 시 client별 token으로 분리해 revoke 가능 (ADR-030).
...
본인 기준:
- Mac mini의 Codex CLI exec가 CoreMCP를 통해 GitHub/Notion 등 실제 MCP를 사용한다.
- 필요 시 MacBook의 Claude Code가 Tailscale로 동일 도구함을 쓴다.
- downstream MCP의 schema 변경이 자동 감지된다.
- credential은 평문으로 어디에도 저장되지 않는다.
- Mac mini 재부팅 후 자동 복귀한다.
- 본인 외에는 어떤 client도 접근 불가하다.
- Codex CLI exec와 선택 client가 각각 별도 client token으로 동작하고, 한쪽 revoke 시 다른 쪽은 영향 없다.
...
| External Connection | Codex CLI exec, Claude Code, ChatGPT 등 외부 client 등록 |
