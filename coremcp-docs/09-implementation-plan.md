...
목표: Admin token 하나로 Claude Code → fake tool 호출까지의 가장 단순한 end-to-end.
...
- [ ] Claude Code 로컬 등록 (admin token으로) + tools/list / tools/call 성공 확인

Exit Criteria:
- `claude mcp add ... --header "Authorization: Bearer <admin>"` 후 fake tool 호출 성공
- invocation log 한 줄
...
- [ ] credential vault (KeychainBackend primary, FernetBackend fallback)
...
Tasks:
- [ ] Next.js scaffold (App Router) + Tailwind + shadcn/ui
- [ ] Admin Token 입력 화면 + localStorage
- [ ] API client wrapper (TanStack Query + fetch)
- [ ] Dashboard page (services count, recent invocations, system health)
- [ ] Services list / new / detail (Overview/Tools/Validation/Credential/Logs/Settings 탭)
- [ ] Toolbox page (drag-and-drop 옵션)
- [ ] Playground page (tool 직접 호출)
- [ ] Logs invocations / audit
- [ ] Connected clients 목록 + revoke
- [ ] Connect Claude Code 가이드 (Mac mini local + Tailscale 명령 generator)
- [ ] Settings: token rotate, locale, debug trace
- [ ] Settings / Tokens 페이지 (Admin + Client 분리, ADR-030)
- [ ] Connected Clients에 client token prefix + revoke 버튼
- [ ] Client token 발급 modal (평문 1회 노출)
- [ ] Error boundary + 401 toast
...
Tasks:
- [ ] launchd plist (com.coremcp.api / com.coremcp.web)
- [ ] Mac mini boot 후 자동 시작 검증
- [ ] daily SQLite backup script + cron
- [ ] log rotation (daily, 7일 보관)
- [ ] Tailscale Serve 또는 Caddy reverse proxy 설정 (옵션)
- [ ] listChanged SSE emission (toolbox 변경 / schema 변경 시)
- [ ] cancellation forward
- [ ] idempotency_key 캐시
- [ ] one-time connection token 발급 / exchange (OpenClaw용)
- [ ] /v1/playground/tools/call
- [ ] OAuth 2.1 자체 AS (옵션, AUTH_MODE=oauth)
- [ ] CIMD endpoint handler (AUTH_MODE=oauth 활성 시, ADR-036)
- [ ] CIMD client metadata fetch + 캐시 (ADR-036)
- [ ] DCR fallback handler
- [ ] icons CDN 캐시 (옵션)
- [ ] (옵션) PostgreSQL 마이그레이션 dry-run
- [ ] (옵션) Prometheus /metrics endpoint
- [ ] (옵션) Sentry / OTel 통합
- [ ] Mac mini 재부팅 후 5분 이내 서비스 정상화 확인

Exit Criteria:
- Mac mini 무인 운영 1주 + Claude Code 통한 일상 사용 안정.
...
7. claude mcp add --transport http coremcp http://localhost:8787/mcp \
     --header "Authorization: Bearer $(cat ~/.coremcp/admin-token)"
8. Claude Code: "fake.add 사용해서 1+2 계산해" → 3 응답
```
...
- P0: Claude Code → fake tool 호출 성공 영상/스크린샷
- P1: 실제 GitHub MCP 연결 후 issue 생성 영상
- P1: Mac mini local Claude Code(admin token) → MacBook Claude Code(client token) → 동일 도구함 동작 + 한쪽 revoke 시연
