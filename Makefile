...
.PHONY: bootstrap run run-local stop status test test-api test-fake test-demo test-project-docs test-git-workspace demo-run lint build smoke ops-smoke external-env-validate soak-check mobile-qa-checklist route-smoke cli-doctor cli-service-list cli-token-issue cli-backup-export cli-backup-import-dry-run backup-restore-drill project-docs-register git-workspace-register codex-install codex-smoke codex-exec
